...
void CGM_NFC::read_FRAM(std::vector<uint8_t>& uid, const uint8_t block_size, std::vector<uint8_t>& buffer) {
    auto num_blocks = 43;

    for (auto num = 0; num < num_blocks; num++) {
        uint8_t block_data[block_size];

        auto rc = readSingleBlock(uid.data(), num, block_data, block_size);
